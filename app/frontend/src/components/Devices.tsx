...
import { RadioTower, Sliders, RotateCcw, ChevronDown, ChevronRight, SatelliteDish } from "lucide-react";
...
function ScanCard({ defaults, onSaved }: { defaults?: ScanSettings; onSaved: () => void }) {
  ...
  return (
    <Card>
      <CardHeader title="Default scan settings" icon={<Sliders size={16} />}
        subtitle="The baseline every dongle inherits unless it overrides a field. Capture hot-restarts affected receivers within seconds." />
      <CardBody>
...
function DeviceCard({ d, defaults, liveRate, onChange }: { d: Device; defaults: ScanSettings; liveRate: number; onChange: () => void }) {
  ...
  return (
    <div className={"rounded-xl border border-border bg-surface p-4 shadow-card " + (cfg.enabled ? "" : "opacity-60")}>
      <div className="flex items-start justify-between">
        <div className="min-w-0">
          <div className="truncate font-medium">{d.label || d.name || "RTL-SDR"}</div>
      ...
      <button onClick={() => setOpen(!open)} className="mt-3 inline-flex items-center gap-1 text-micro text-brand hover:underline">
        {open ? <ChevronDown size={13} /> : <ChevronRight size={13} />} scan settings
      </button>
      {open && (
        <div className="mt-2 space-y-3 rounded-lg border border-border bg-app/40 p-3">
          <p className="text-micro text-tertiary">Blank = inherit the default. Set a field to override it just for this dongle.</p>
          ...
          <div><div className="label mb-1">Message types <span className="lowercase text-tertiary">(default: {defaults.msgtype})</span></div>
            <MsgTypeChips value={cfg.msgtype || defaults.msgtype} onChange={(v) => setCfg({ ...cfg, msgtype: v })} /></div>
          <Field label="Label"><Input value={cfg.label || ""} onChange={(e) => setCfg({ ...cfg, label: e.target.value })} placeholder="friendly name" /></Field>
          <div className="flex gap-2">
            <Button size="sm" variant="primary" onClick={() => save(cfg)} success="Applied — dongle restarting">Apply to this dongle</Button>
            <Button size="sm" variant="ghost" icon={<RotateCcw size={13} />} onClick={() => save({ enabled: cfg.enabled, label: cfg.label, freq: "", gain: "", ppm: "", msgtype: "", filterid: "" })} success="Reset to defaults">Use defaults</Button>
          </div>
        </div>
      )}
    </div>
...
function Coverage() {
  ...
  return (
    <Card>
      <CardHeader title="Reception coverage" icon={<SatelliteDish size={16} />} subtitle="Which dongle hears which meter (all-time). Use it to place/aim your SDRs." />
      <CardBody>
        <div className="mb-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
          {perSource.map((p) => (
            <div key={p.s} className="rounded-lg border border-border bg-app/40 px-3 py-2">
                ...
                <tr key={m} className="border-t border-border/50">
                  ...
                  {sources.map((s) => { const v = get[`${s}-${m}`] || 0; return (
                    <td key={s} className="px-1 py-1 text-center">
                      <span className="inline-block w-full rounded px-1 tabular-nums" style={{ background: v ? `rgba(45,212,191,${0.1 + (v / max) * 0.7})` : "transparent", color: v ? "#04241f" : "#475569" }}>{v || "·"}</span>
                    </td>); })}
                </tr>
              ))}
            </tbody>
